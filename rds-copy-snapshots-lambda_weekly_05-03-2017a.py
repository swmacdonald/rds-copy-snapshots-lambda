...
#					Filters=filters,
					MaxRecords=80)['DBSnapshots']
#	filter the snapshots based on the the first letters of the DBSnapshotIdentifier
#	filter(['DBSnapshotIdentifier'].startswith(str_snap_name_prefx), snapshots)
	snapshots=filter(lambda x: x['DBSnapshotIdentifier'].startswith(str_snap_name_prefx), snapshots)	
	print(sorted(snapshots, key=byTimestamp, reverse=False))
